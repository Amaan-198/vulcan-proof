...
check("order screen starts without a selected row", await page.locator(".order-table tbody tr.selected").count() === 0 && await page.getByText("Select an order from the table.").isVisible());
await page.locator(".order-table tbody tr").first().click();
check("row selection requires an explicit click", await page.locator(".order-table tbody tr.selected").count() === 1);
...
await page.waitForFunction(() => document.querySelectorAll(".order-table tbody tr").length > 0, { timeout: 30000 });
check("category filter leaves visible rows", await page.locator(".order-table tbody tr").count() > 0);
check("category filter clears the selection", await page.locator(".order-table tbody tr.selected").count() === 0 && await page.getByText("Select an order from the table.").isVisible());
await page.locator(".order-table tbody tr").first().click();
...
check("filtered row can be selected explicitly", selectedCategory?.trim() === "Apparel", selectedCategory?.trim() || "");
